...
var aqiCharWrap = document.getElementsByClassName('aqi-chart-wrap')[0];
var px = 0;//用于按日 周 月 设置margin-right;

function renderChart() {
  var color = '',text = '';
  for (var item in chartData){
    color = '#' + Math.floor(Math.random() * 0xFFFFFF).toString(16);
    text +='<div title="'+item+":"+chartData[item]+'" style="height:'+chartData[item]+'px;margin-right:'+px+'px; background-color:'+color+'"></div>';
  }
...
function initGraTimeForm() {
  ...
  for (var i=0; i< pageRadio.length; i++) {
    addEventHandle(pageRadio[i],'click',graTimeChange);
  }
}
...
function initCitySelector() {
  ...
  addEventHandle(citySelect,'change',citySelectChange);
}
...
function initAqiChartData() {
  ...
  //nowCityData是确定的一个城市91天降水数组,key是日期，nowCityData[key]是降水量
